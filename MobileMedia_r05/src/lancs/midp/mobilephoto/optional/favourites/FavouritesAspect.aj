...
public aspect FavouritesAspect {
	...
	boolean around(PhotoController controller, Command c): handleCommandAction(controller, c) {
		...
		if (label.equals("Set Favorite")) {
			...
			try {
				ImageData image = controller.getAlbumData().getImageAccessor().getImageInfo(selectedImageName);
	...
	pointcut append(PhotoListController controller, ImageData image):
		call(public int PhotoListScreen.append(ImageData)) && args(image) && this(controller);
	
	int around(PhotoListController controller, ImageData image) : append(controller, image) {
//		System.out.println("<* FavouritesAspect.around append *> begins... ");
	...
	after(PhotoListController controller, ImageData[] images, PhotoListScreen imageList): appendImages(controller, images, imageList) {
